...
uint8_t BoardSolver::calculate_best_move(const Board& board, uint8_t max_moves)
{
	...
	for (uint8_t cell = 0; cell < BOARD_WIDTH; cell++)
	{
		...
		int32_t score = minimax(new_board, max_moves - 1, true);
		if (score > best_score)
		{
			best_score = score;
...
int32_t BoardSolver::minimax(const Board& board, uint8_t max_depth, bool maximizing_player)
{
	...
	for (uint8_t cell = 0; cell < BOARD_WIDTH; cell++)
	{
		...
		int32_t score = minimax(new_board, max_depth - 1, !maximizing_player);
